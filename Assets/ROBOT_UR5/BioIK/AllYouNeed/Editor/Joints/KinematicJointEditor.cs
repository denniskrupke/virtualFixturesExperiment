	...
	public class KinematicJoint_CE : Editor {
		...
		public override void OnInspectorGUI() {
			...
			using (var scope = new EditorGUILayout.VerticalScope ("Button")) {
				...
				Vector3 defaultPosition = EditorGUILayout.Vector3Field("Position", Target.GetDefaultPosition());
				Quaternion defaultRotation = Quaternion.Euler(EditorGUILayout.Vector3Field("Rotation", Target.GetDefaultRotation().eulerAngles));
				Target.SetDefaultFrame(defaultPosition, defaultRotation);
			}
			...
			using (var scope = new EditorGUILayout.VerticalScope ("Button")) {
				...
				Target.SetAnimationWeight(EditorGUILayout.Slider("Animation Weight", Target.GetAnimationWeight(), 0f, 1f));
				Target.SetAnimationBlend(EditorGUILayout.Slider("Animation Blend", Target.GetAnimationBlend(), 0f, 1f));
			...
			using (var scope = new EditorGUILayout.VerticalScope ("Box")) {
				...
				if(motion.IsEnabled()) {
					if(motion.Joint.GetJointType() != JointType.Continuous) {
						motion.SetLowerLimit(Setter()*EditorGUILayout.FloatField("Lower Limit", Getter()*motion.GetLowerLimit()));
						motion.SetUpperLimit(Setter()*EditorGUILayout.FloatField("Upper Limit", Getter()*motion.GetUpperLimit()));
					}
					motion.SetTargetValue(Setter()*EditorGUILayout.Slider("Target Value", Getter()*motion.GetTargetValue(), Getter()*motion.GetLowerLimit(), Getter()*motion.GetUpperLimit()));
				}
			}
		}

		private float Setter() {
			return Target.GetJointType() == JointType.Prismatic ? 1f : Mathf.Deg2Rad;
		}

		private float Getter() {
			return Target.GetJointType() == JointType.Prismatic ? 1f : Mathf.Rad2Deg;
		}
		...
		void OnSceneGUI() {
			...
			Handles.SphereHandleCap(0, anchor, Quaternion.identity, 1f/100f, EventType.Repaint);
			...
			if(Target.GetXMotion().IsEnabled()) {
				...
				if(Target.GetJointType() == JointType.Prismatic) {
					...
					Handles.CubeHandleCap(0, A, rotation, 0.025f, EventType.Repaint);
					Handles.DrawLine(anchor, B);
					Handles.CubeHandleCap(0, B, rotation, 0.025f, EventType.Repaint);
				} else {
			...
			Handles.ArrowHandleCap(0, anchor, rotation * Quaternion.LookRotation(Target.GetXMotion().Axis), 0.25f, EventType.Repaint);
			...
			Handles.ArrowHandleCap(0, anchor, rotation * Quaternion.LookRotation(Target.GetYMotion().Axis), 0.25f, EventType.Repaint);
			...
			Handles.ArrowHandleCap(0, anchor, rotation * Quaternion.LookRotation(Target.GetZMotion().Axis), 0.25f, EventType.Repaint);
		}
	}
}